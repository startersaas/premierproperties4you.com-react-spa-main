...
const TopIconsPrivate = ({ 
  isXs, 
  isHomePage, 
  useTheme: propsUseTheme, 
  user,
  ...
  return (
    <AppBar position="static" elevation={0} color="transparent">
      <Toolbar ref={toolbarRef} sx={{ 
        ...
        {/* Only show chips and buttons if hideButtons is false */}
        {!hideButtons && (
      ...
      {/* User Menu - still shown even if buttons are hidden */}
      {user && (
        <Menu 
      ...
      {/* Portal Button that appears when scrolled down - only shown if hideButtons is false */}
